...
tags:
  - configuration
  - docs
description: How you can make the AstroPaper+ theme absolutely yours (fork of AstroPaper by Sat Naing).
---
import ResponsiveTable from '@/components/ResponsiveTable.astro';

This guide covers the available configuration options in **AstroPaper+** — from site metadata and feature flags to fonts, social links, and layout settings. AstroPaper+ is a fork of AstroPaper by [Sat Naing](https://github.com/satnaing); see the [about page](/about) for fork attribution.
...
| `dateFormat.post` / `dateFormat.project` | `Intl.DateTimeFormatOptions` for the post-detail / listing dates and the project-card timestamps respectively. Locale-specific CLDR overrides are picked up automatically for every supported locale. |
...
| Option                | Description                                                                                                                       |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `scheduledPostMargin` | Posts, galleries and projects with a future `pubDatetime` within this window (in ms) are treated as published. Defaults to 15 minutes (`15 * 60 * 1000`). |
...
| `dynamicOgImage`   | Generate a dynamic OG image per post when no `ogImage` is specified in frontmatter. Defaults to `true`. See the [trade-off](/posts/dynamic-og-image-generation-in-astropaper-blog-posts/) for details.                                       |
...
- [Customizing AstroPaper+ theme color schemes](/posts/customizing-astropaper-theme-color-schemes/) — change or add color schemes via `src/styles/theme.css`.
- [Adding new posts](/posts/adding-new-posts-in-astropaper-theme/) — frontmatter reference and file conventions.
