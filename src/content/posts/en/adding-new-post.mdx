...
tags:
  - docs
description: "Some rules & recommendations for creating or adding new posts using the AstroPaper+ theme (fork of AstroPaper by Sat Naing)."
---
import ResponsiveTable from '@/components/ResponsiveTable.astro';

This guide covers the rules and conventions for creating new posts in AstroPaper+ — file placement, frontmatter fields, images, and syntax highlighting. AstroPaper+ is a fork of AstroPaper by [Sat Naing](https://github.com/satnaing); see the [About page](/about) for fork attribution.
...
| **_description_**  | Description of the post. Used in the post excerpt and the page's SEO meta tags. If omitted, AstroPaper+ falls back to the body excerpt up to a `<!-- more -->` marker. | optional (recommended for SEO)                 |
...
Only `title` and `pubDatetime` are required in frontmatter. `description` is optional — when it's missing, AstroPaper+ extracts the post excerpt from the body up to a `<!-- more -->` separator (same convention as Jekyll/Hugo/Eleventy). Title and description (excerpt) are still important for search engine optimization (SEO), so AstroPaper+ recommends including either a frontmatter `description:` or a `<!-- more -->` marker in every blog post.
...
AstroPaper+ started supporting callouts in upstream AstroPaper v6.1, and they are present in every release of AstroPaper+ from v6.1.0 onward (including **AstroPaper+ v7.0.0**, this fork release). They use a simple blockquote syntax powered by `rehype-callouts` (Obsidian theme).
...
Here are some recommendations, tips & tricks for creating new posts in the AstroPaper+ blog theme.
...
> [!TIP]
> Since AstroPaper+ v1.4.0, OG images are generated automatically if not specified. Check out [the announcement](/posts/dynamic-og-image-generation-in-astropaper-blog-posts/).
